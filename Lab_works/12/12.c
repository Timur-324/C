...
{
    char *p = result + BUFSIZ - 1; /*ставлю указатель в конец для того, чтобы получаемые числа записывать с конца*/
    *p-- = 0; /*смещаю указатель с нуля на предпоследнюю ячейку*/
    ...
    {
        int r = num % sys_num; /*остаток от деления - цифра в новой системе*/
        *p-- = (r > 9) ? (r - 10 + 'A') : (r + '0');
